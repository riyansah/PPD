...
function createApiRouter({ env, db, authService, dashboardService, reportService, taskService, activityService, routineService }) {
  const router = express.Router();

  router.use("/auth", createAuthRouter({ env, authService }));
  router.use("/dashboard", createDashboardRouter({ dashboardService }));
  router.use("/reports", createReportRouter({ reportService }));
